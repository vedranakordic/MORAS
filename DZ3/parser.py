...
class Parser:
    ...
    from parseSymbols import _parse_symbols, _parse_symbol, _init_labels, _parse_variable, _parse_macro
    from parsCom import _parse_commands, _parse_command, _init_comms
        ...
        for (line, p, o) in self._lines:
            ...
            if (len(newline) > 0):
                ...
                if type(newline) == list:
                    for k in newline:
                #iteriram po newline i ubacujem u newlines
                # zbog vracanja naredbi u obliku liste
                # kao jedan element
